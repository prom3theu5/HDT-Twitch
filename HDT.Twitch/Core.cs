...
namespace Twitch
{
	public class Core
	{
		...
		public static bool Connect()
		{
            ...
            var credentials = new ConnectionCredentials(Config.Instance.User, Config.Instance.OAuth);
            _client = new TwitchClient(credentials, Config.Instance.User);
        ...
        private static void _client_OnMessageReceived(object sender, TwitchLib.Events.Client.OnMessageReceivedArgs e)
        {
            if (!e.ChatMessage.Message.StartsWith("!"))
                return;
            var cmd = e.ChatMessage.Message.Substring(1);
